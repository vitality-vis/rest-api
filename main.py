...
# from service.infrastructure.zilliz import query_doc_by_ids, query_docs, normalize_results
# import config
# # from service.application.agent_service import build_agent, run_two_stage_hybrid_rag  # include new two-stage RAG
# # from service.application.agent_service import build_agent, run_two_stage_rag
...
# from service.application.agent_service import run_two_stage_rag_stream
# import asyncio
...
# from flask import Response, request
# from flask_cors import cross_origin
# # from service.application.agent_service import streaming_llm  
# import asyncio
...
# from service.application.agent_service import reset_all_sessions
...
from service.application.zotero_service import (
    ZoteroServiceError,
    connect_zotero_library,
    list_zotero_library_items,
    import_zotero_pdf_attachment,
)
...
# ===== Literature Review (LR) human-in-the-loop chat blueprint =====
# Additive route only (POST /lrChat); does NOT touch /chat or the RAG graph.
# Guarded so the server still boots if `langgraph` isn't installed yet.
try:
    from service.agents.lr.lr_routes import lr_bp
    app.register_blueprint(lr_bp)
    logger.info("Registered LR chat blueprint (/lrChat).")
except Exception as _lr_exc:  # noqa: BLE001
    logger.warning("LR chat blueprint not registered (%s). "
                   "Install `langgraph` to enable /lrChat.", _lr_exc)
...
def _rank_global_match(doc: dict, needle: str) -> tuple:
    """Lower tuple sorts earlier. Prioritize title startswith/contains, then source/authors/abstract."""
    title = str(doc.get("Title") or "").lower()
    source = str(doc.get("Source") or "").lower()
    abstract = str(doc.get("Abstract") or "").lower()
    authors = [str(a).lower() for a in (doc.get("Authors") or [])]

    title_starts = 0 if title.startswith(needle) else 1
    title_idx = title.find(needle) if needle in title else 10**6
    source_idx = source.find(needle) if needle in source else 10**6
    author_idx = min((a.find(needle) for a in authors if needle in a), default=10**6)
    abstract_idx = abstract.find(needle) if needle in abstract else 10**6
    return (title_starts, title_idx, source_idx, author_idx, abstract_idx, len(title))


@app.route('/searchCorpus', methods=['POST'])
@cross_origin()
def search_corpus():
    """
    Global corpus search across full cached dataset without preloading papers in frontend.
    Matches query in Title, Abstract, Authors, Source (OR logic), with lightweight ranking.
    """
    input_payload = request.json or {}
    query_text = str(input_payload.get('query', '')).strip()
    limit = int(input_payload.get('limit', 1000))
    offset = int(input_payload.get('offset', 0))
    ...
    all_papers = zilliz.query_docs(QuerySchema(limit=-1)).get("papers", [])
    if not all_papers:
        return jsonify({"papers": [], "total": 0})
    ...
    for paper in all_papers:
        ...
        if (
            _contains_case_insensitive(paper.get("Title"), needle)
            or _contains_case_insensitive(paper.get("Abstract"), needle)
            or _contains_case_insensitive(paper.get("Source"), needle)
            or any(_contains_case_insensitive(a, needle) for a in authors)
        ):
            matched.append(paper)

    matched.sort(key=lambda doc: _rank_global_match(doc, needle))
    total_count = len(matched)
    if limit == -1:
        page = matched[offset:]
    else:
        page = matched[offset : offset + limit]
    return jsonify({"papers": page, "total": total_count})
...
def chat():
    ...
    text = data.get('text', '').strip()
    chat_id = data.get('chat_id', 'default')
    selected_paper_ids = data.get('selected_paper_ids') or data.get('paper_ids') or []
    selected_paper_titles = data.get('selected_paper_titles') or data.get('paper_titles') or []
    ...
    async def agen():
        async for chunk in run_two_stage_rag_stream(
            text, chat_id,
            selected_paper_ids=selected_paper_ids or None,
            selected_paper_titles=selected_paper_titles or None,
        ):
            yield chunk

    # Sync wrapper: on any exception, yield fallback so client gets 200 + content
    def stream_sync():
        # Prime the browser's fetch() ReadableStream: some browsers withhold the
        # first ~1KB of a response before surfacing any bytes to the reader,
        # which delays the early tiny [SIGNAL:STATUS:...] tokens so the whole
        # "thinking" timeline appears at once. A whitespace prelude (the client
        # ignores whitespace-only chunks) forces that buffer to flush
        # immediately so each status step renders in real time.
        yield " " * 2048 + "\n"
        try:
            agen_obj = agen().__aiter__()
            while True:
    ...
    # Stream in real time: browsers buffer text/plain responses (holding the
    # first ~1KB for MIME-sniffing) before surfacing bytes to a fetch()
    # ReadableStream. That makes the early, tiny [SIGNAL:STATUS:...] tokens
    # appear all at once instead of progressively. `nosniff` disables that
    # buffering; the other headers defeat proxy/nginx response buffering.
...
def _zotero_error_response(exc: ZoteroServiceError):
    return jsonify({
        "status": "error",
        "code": exc.code,
        "message": exc.message,
...
@app.route("/zotero/connect", methods=["POST"])
@cross_origin()
def zotero_connect():
    data = request.json or {}
    try:
        result = connect_zotero_library(
            chat_id=data.get("chat_id"),
            zotero_user_id=data.get("zotero_user_id") or data.get("zoteroUserId"),
            zotero_api_key=data.get("zotero_api_key") or data.get("zoteroApiKey"),
            app_user_id=data.get("user_id"),
        )
        return jsonify({"status": "success", **result})
    except ZoteroServiceError as exc:
        return _zotero_error_response(exc)
    except Exception as exc:
        logger.exception("Unexpected Zotero connect error")
        return jsonify({
            "status": "error",
            "code": "zotero_internal_error",
            "message": str(exc),
        }), 500


@app.route("/zotero/items", methods=["POST"])
@cross_origin()
def zotero_items():
    data = request.json or {}
    try:
        result = list_zotero_library_items(
            connection_id=data.get("connection_id") or data.get("connectionId"),
            chat_id=data.get("chat_id"),
            app_user_id=data.get("user_id"),
            collection_key=data.get("collection_key") or data.get("collectionKey"),
        ...
        logger.exception("Unexpected Zotero items error")
        return jsonify({
            "status": "error",
            "code": "zotero_internal_error",
            "message": str(exc),
...
@app.route("/zotero/import-pdf", methods=["POST"])
@cross_origin()
def zotero_import_pdf():
    data = request.json or {}
    try:
        result = import_zotero_pdf_attachment(
            connection_id=data.get("connection_id") or data.get("connectionId"),
            chat_id=data.get("chat_id"),
            app_user_id=data.get("user_id"),
            item_key=data.get("item_key") or data.get("itemKey"),
            attachment_key=data.get("attachment_key") or data.get("attachmentKey"),
        )
        return jsonify({"status": "success", **result})
    except ZoteroServiceError as exc:
        return _zotero_error_response(exc)
    except Exception as exc:
        logger.exception("Unexpected Zotero import error")
        return jsonify({
            "status": "error",
            "code": "zotero_internal_error",
            "message": str(exc),
...
if __name__ == "__main__":
    ...
    socketio.run(app, host="0.0.0.0", port=port, debug=debug_mode, use_reloader=debug_mode, allow_unsafe_werkzeug=True)
