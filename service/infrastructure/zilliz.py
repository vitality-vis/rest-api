...
from service.utils.metadata_normalizer import parse_string_list
...
ZOTERO_CHUNK_COLLECTION = getattr(config, "ZOTERO_CHUNK_COLLECTION", "zotero_chunks_glove")
...
def _get_collection(collection_name: str):
    ...
    try:
        if not utility.has_collection(collection_name):
            logging.error(f"Collection '{collection_name}' does not exist. Run scripts/load_to_zilliz.py first.")
            return None
...
def _create_zotero_chunk_schema(dim: int):
    from pymilvus import CollectionSchema, FieldSchema, DataType

    fields = [
        FieldSchema(name="ID", dtype=DataType.VARCHAR, max_length=256, is_primary=True),
        FieldSchema(name="embedding", dtype=DataType.FLOAT_VECTOR, dim=dim),
        FieldSchema(name="ScopeID", dtype=DataType.VARCHAR, max_length=512),
        FieldSchema(name="ChatID", dtype=DataType.VARCHAR, max_length=256),
        FieldSchema(name="AppUserID", dtype=DataType.VARCHAR, max_length=256),
        FieldSchema(name="ZoteroUserID", dtype=DataType.VARCHAR, max_length=64),
        FieldSchema(name="ParentItemKey", dtype=DataType.VARCHAR, max_length=64),
        FieldSchema(name="AttachmentKey", dtype=DataType.VARCHAR, max_length=64),
        ...
        FieldSchema(name="ContentType", dtype=DataType.VARCHAR, max_length=128),
        FieldSchema(name="ChunkIndex", dtype=DataType.INT64),
        FieldSchema(name="ChunkText", dtype=DataType.VARCHAR, max_length=65535),
    ]
    return CollectionSchema(fields=fields, description="zotero attachment chunks")


def _get_or_create_zotero_chunk_collection(dim: int):
    if not _ensure_connection():
        return None
    pym = _get_pymilvus()
    if not pym:
    ...
    _, Collection, utility = pym
    try:
        if utility.has_collection(ZOTERO_CHUNK_COLLECTION):
            coll = Collection(ZOTERO_CHUNK_COLLECTION)
            coll.load()
            return coll

        coll = Collection(
            name=ZOTERO_CHUNK_COLLECTION,
            schema=_create_zotero_chunk_schema(dim),
        )
        index_type = getattr(config, "ZILLIZ_INDEX_TYPE", "HNSW")
        ...
        coll.load()
        logging.info("Created Zotero chunk collection: %s", ZOTERO_CHUNK_COLLECTION)
        return coll
    except Exception as e:
        logging.error(
            "Failed to get or create Zotero chunk collection '%s': %s",
            ZOTERO_CHUNK_COLLECTION,
            e,
            exc_info=True,
        )
        return None
...
def zotero_chunk_count(scope_id: str, attachment_key: Optional[str] = None) -> int:
    """Return the number of Zotero chunk rows stored for a given scope and attachment."""
    coll = _get_or_create_zotero_chunk_collection(dim=384)
    if not coll:
        return 0

    expr = f'ScopeID == "{_expr_escape(scope_id)}"'
    if attachment_key:
        expr += f' and AttachmentKey == "{_expr_escape(attachment_key)}"'

    try:
        rows = coll.query(expr=expr, output_fields=["ID"], limit=16384)
        return len(rows or [])
    except Exception as e:
        logging.error("Failed to count Zotero chunks: %s", e, exc_info=True)
        return 0


def insert_zotero_chunks(records: List[Dict[str, Any]]) -> int:
    """Insert normalized Zotero PDF chunks into a dedicated collection."""
    if not records:
        return 0

    dim = len(records[0].get("embedding") or [])
    if dim <= 0:
        raise ValueError("Zotero chunk embeddings are empty.")

    coll = _get_or_create_zotero_chunk_collection(dim=dim)
    if not coll:
        raise RuntimeError("Zotero chunk collection is unavailable.")

    def _s(value: Any, max_len: int) -> str:
        return str(value or "")[: max(1, max_len - 1)]

    try:
        coll.insert([
            [_s(rec.get("id"), 256) for rec in records],
            [rec["embedding"] for rec in records],
            [_s(rec.get("scope_id"), 512) for rec in records],
            [_s(rec.get("chat_id"), 256) for rec in records],
            [_s(rec.get("app_user_id"), 256) for rec in records],
            [_s(rec.get("zotero_user_id"), 64) for rec in records],
            [_s(rec.get("parent_item_key"), 64) for rec in records],
            [_s(rec.get("attachment_key"), 64) for rec in records],
            [_s(rec.get("title"), 2048) for rec in records],
            [_s(rec.get("filename"), 1024) for rec in records],
            [_s(rec.get("content_type"), 128) for rec in records],
            [int(rec.get("chunk_index", 0)) for rec in records],
            [_s(rec.get("chunk_text"), 65535) for rec in records],
        ])
        try:
            coll.flush()
        except Exception:
        ...
        logging.error("Failed to insert Zotero chunks: %s", e, exc_info=True)
        raise
...
def get_aggregated_metadata(
    ...
    titles = sorted(set(str(doc.get("Title")).strip() for doc in docs if doc.get("Title")))
