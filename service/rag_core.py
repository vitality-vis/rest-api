...
# def _run_semantic_search(query_text: str, chat_id: str, top_k: int = 5) -> List[Document]:
#     """Run semantic search with reranking and save docs per chat."""
#     recall = _query_chroma_by_embedding(query_text, "specter", k=200)
#     if not recall:
#         return []

#     # Compute cosine scores
#     embedder = EMBEDDING_MODELS["specter"]
#     qvec = np.array(embedder.embed_query(query_text))
#     for d in recall:
#         text = d.page_content or d.metadata.get("abstract", "")
#         dvec = np.array(embedder.embed_query(text))
#         denom = np.linalg.norm(qvec) * np.linalg.norm(dvec)
#         score = np.dot(qvec, dvec) / denom if denom else 0
#         d.metadata["_cosine_score"] = score

#     recall.sort(key=lambda d: d.metadata["_cosine_score"], reverse=True)
#     top_ranked = recall[:50]

#     # Rerank with cross-encoder
#     pairs = [(query_text, d.page_content or d.metadata.get("abstract", "")) for d in top_ranked]
#     scores = CROSS_ENCODER_MODEL.predict(pairs)
#     for d, s in zip(top_ranked, scores):
#         d.metadata["_rerank_score"] = float(s)
#     top_ranked.sort(key=lambda d: d.metadata["_rerank_score"], reverse=True)
#     final_docs = top_ranked[:top_k]
...
# def hybrid_refine(meta_docs, sem_docs, alpha=0.7, top_k=5):
#     """
#     Intersection-based hybrid refinement:
#     Boosts documents that appear in both metadata and semantic search.
#     Returns (top_docs, found_overlap)
#     """
#     meta_ids = {d.metadata["id"] for d in meta_docs}
#     sem_ids = {d.metadata["id"] for d in sem_docs}
#     intersection_ids = meta_ids.intersection(sem_ids)

#     merged = []
#     for d in sem_docs:
#         doc_id = d.metadata["id"]
#         base_score = d.metadata.get("_score", 0)
#         d.metadata["_hybrid_score"] = alpha * base_score + (1 - alpha) if doc_id in intersection_ids else alpha * base_score
#         merged.append(d)

#     merged.sort(key=lambda d: d.metadata["_hybrid_score"], reverse=True)
#     top_docs = [d for d in merged if d.metadata["id"] in intersection_ids][:top_k]
#     found_overlap = len(top_docs) > 0
#     return top_docs, found_overlap




# def _run_precision_search(query_text: str, top_k: int = 5) -> List[Document]:
#     # 1. Run Metadata Search (High Precision, Low Recall)
#     # This finds ANY paper by "Tim" regardless of topic
#     meta_docs = _query_chroma_by_metadata(query_text) 
#     meta_ids = {d.metadata.get("paper_id") for d in meta_docs}

#     # 2. Run Semantic Search (High Recall)
#     # This finds papers about "Data Mining" regardless of author
#     sem_docs = _query_chroma_by_embedding(query_text, "specter", k=50)
    
#     # 3. Strict Intersection (The "Perfect Match")
#     perfect_matches = []
#     for d in sem_docs:
#         if d.metadata.get("paper_id") in meta_ids:
#             d.metadata["_match_type"] = "perfect_match"  # Author + Topic
#             perfect_matches.append(d)
            
#     # === LOGIC BRANCH ===
    
#     # Case A: We found perfect matches (Precision Goal Met)
#     if perfect_matches:
#         # Rerank ONLY the perfect matches to find the best ones among them
#         return _rerank_docs(perfect_matches[:top_k], query_text)

#     # Case B: No Intersection (Tim has no Data Mining papers)
#     # You said precision is key. Returning random stuff is bad.
#     # Strategy: Return the Semantic results (Topic matches), but LABEL them clearly.
    
#     print("⚠️ No exact matches found. Falling back to topic search.")
    
#     # We take the top semantic docs (Alice's Data Mining papers)
#     # But we add a metadata flag so your UI can say: "No papers by Tim found. Here are similar papers by others."
#     fallback_docs = sem_docs[:top_k]
#     for d in fallback_docs:
#         d.metadata["_match_type"] = "topic_fallback"
        
#     return _rerank_docs(fallback_docs, query_text)

# def hybrid_refine(docs, query):
#     """Helper to run your Cross-Encoder on a list"""
#     if not docs: return []
#     pairs = [(query, d.page_content) for d in docs]
#     scores = CROSS_ENCODER_MODEL.predict(pairs)
#     for d, s in zip(docs, scores):
#         d.metadata["_rerank_score"] = float(s)
#     docs.sort(key=lambda x: x.metadata["_rerank_score"], reverse=True)
#     return docs


# def hybrid_refine(meta_docs, sem_docs, query_text, top_k=5, alpha=0.7):
#     """
#     Waterfall strategy hybrid refinement (same philosophy as _run_precision_search):
#         1. Compute the intersection (metadata ∩ semantic)
#         2. If intersection exists → treat as perfect matches
#         3. If no intersection → fallback to semantic results
#         4. Rerank using cross-encoder
#         5. Return (final_docs, found_overlap)
#     """

#     # --- 1. Build ID sets ---
#     meta_ids = {d.metadata.get("paper_id") for d in meta_docs}
#     sem_ids  = {d.metadata.get("paper_id") for d in sem_docs}

#     intersection_ids = meta_ids & sem_ids
#     found_overlap = len(intersection_ids) > 0

#     # --- 2. Waterfall branch A: PERFECT MATCHES ---
#     if found_overlap:
#         final_docs = []
#         for d in sem_docs:  # semantic docs typically carry better content and embedding score
#             if d.metadata.get("paper_id") in intersection_ids:
#                 d.metadata["_match_type"] = "perfect_match"
#                 final_docs.append(d)

#     # --- 3. Waterfall branch B: FALLBACK ---
#     else:
#         final_docs = sem_docs[:top_k]
#         for d in final_docs:
#             d.metadata["_match_type"] = "topic_fallback"

#     # --- 4. Optional: compute hybrid score (semantic + metadata) ---
#     for d in final_docs:
...
#     # # --- 6. Truncate ---
#     # return final_docs[:top_k], found_overlap

#     # --- 6. Truncate and ensure ID consistency ---
#     final_docs = final_docs[:top_k]

#     # FIX: ensure ID always exists (semantic docs only have paper_id)
#     for d in final_docs:
...
# def save_session_docs(chat_id: str, docs: List[Document]) -> None:
#     """Save docs to the correct chat session."""
#     from service.agent_runner import SESSIONS
#     session = SESSIONS.get(chat_id)
#     if session:
#         session["docs"] = deepcopy(docs or [])
#         logging.info(f"[rag_core] Saved {len(docs)} docs to chat_id={chat_id}")
