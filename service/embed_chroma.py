...
class LocalSentenceTransformerEmbedding(Embeddings):
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        # TO THIS (We get the first and only result from the list):
...
# def ada_embedding(input_data: Dict) -> List[float]:
#     """
#     Generates multilingual ADA embeddings using a local SentenceTransformer model.
#     The input_data['abstract'] should contain the abstract text in any supported language.
#     """
#     abstract_text = input_data.get('abstract', '')
#     if not isinstance(abstract_text, str):
#         logging.error(f"[ADA] Invalid abstract_text type: {type(abstract_text)}. Expected str.")
#         return []
...
#     try:
#         embedding = ada_model_instance.embed_query(abstract_text)
#         logging.debug(f"Generated ADA embedding for abstract (length: {len(embedding)})")
#         return embedding
...
# def glove_embedding(input_data: Dict) -> List[float]:
#     """
#     Generates multilingual Glove-like embeddings for general text (e.g., abstract or title)
#     using a local SentenceTransformer model.
#     The input_data['text'] should contain the text to embed in any supported language.
#     """
#     text_to_embed = input_data.get('text', '')
#     if not text_to_embed:
#         logging.warning("No text found for Glove embedding. Returning empty list.")
#         return []
    
#     try:
#         embedding = glove_model_instance.embed_query(text_to_embed)
#         logging.debug(f"Generated Glove embedding for text (length: {len(embedding)})")
#         return embedding
#     except Exception as e:
...
# === Specter Embedding (local Specter model) ===
def specter_embedding(papers: Union[Dict, List[Dict]]) -> List[float]:
    """
    Generates Specter embeddings using the local Specter model.
    This function expects a list of paper dictionaries, each with 'Title' and 'Abstract'.
    It only processes the first paper in the list for simplicity, as per original logic.
    """
...
# AZURE_EMBED_API_VERSION = os.getenv("AZURE_OPENAI_EMBED_API_VERSION", "2024-02-01")
...
#         if torch.cuda.is_available():
#             self.model.cuda()
#             logging.info(f"Local Specter model '{model_name}' moved to GPU.")
#         else:
#             logging.info(f"Loaded local Specter model: {model_name} (on CPU).")
...
# # Instantiate Models
# # ====================
# glove_model_instance = LocalSentenceTransformerEmbedding("sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2")
# specter_model_instance = LocalSpecterEmbedding("allenai/specter")

# # ====================
# # Embedding Functions
# # ====================
# def ada_embedding(text_to_embed) -> List[float]:
#     """
#     Generate 1536-dim Ada embeddings using Azure OpenAI deployment.
#     Accepts either a string, or a dict with 'abstract'.
#     """
#     if isinstance(text_to_embed, dict):
#         text_to_embed = text_to_embed.get("abstract", "")

#     if not text_to_embed or not isinstance(text_to_embed, str):
#         logging.error(f"[Ada] Invalid input for embedding: {text_to_embed}")
#         return []

#     try:
#         response = embed_client.embeddings.create(
#             model=AZURE_EMBED_DEPLOYMENT,
#             input=[text_to_embed]  # must be list
#         )
#         embedding = response.data[0].embedding
#         logging.debug(f"[Ada] Generated embedding length={len(embedding)}")
#         return embedding
#     except Exception as e:
#         logging.error(f"Error generating Ada embedding: {e}")
#         return []

# # def glove_embedding(text_to_embed: str) -> List[float]:
# #     if not text_to_embed:
# #         logging.warning("No text for Glove embedding. Returning empty list.")
# #         return []
# #     return glove_model_instance.embed_query(text_to_embed)

# def glove_embedding(text_to_embed) -> List[float]:
#     """
#     Generate Glove-like embeddings using local SentenceTransformer.
#     Accepts either a string, or a dict with 'text' or 'abstract'.
#     """
#     if isinstance(text_to_embed, dict):
#         text_to_embed = text_to_embed.get("text") or text_to_embed.get("abstract", "")

#     if not text_to_embed or not isinstance(text_to_embed, str):
#         logging.error(f"[Glove] Invalid input for embedding: {text_to_embed}")
#         return []
...
#     combined_text = f"{title} {specter_model_instance.tokenizer.sep_token} {abstract}"
#     if not combined_text.strip():
#         logging.warning("Empty text for Specter embedding.")
#         return []
#     try:
#         return specter_model_instance.embed_query(combined_text)
#     except Exception as e:
#         logging.error(f"Error generating Specter embedding: {e}")
#         return []