...
def format_papers_in_prompt(papers):
    """
    格式化论文列表为 LLM prompt 友好的字符串。
    (这个函数比你旧版 chain.py 里的更健壮，我们用这个新的)
    """
    lines = []
    for p in papers:
        title = p.get('Title', '')
        # 处理 authors 和 keywords 可能不是列表的情况
...
# def stream_to_json_output(generator, key="output"):
#     """
#     将生成器（流）的输出拼接成完整文本，并封装为 JSON 返回。
#     """
#     try:
#         # 拼接所有流输出
#         full_text = "".join(generator)
#         response_data = {key: full_text}
#         return jsonify(response_data) # 直接返回 Flask Response 对象

#     except Exception as e:
#         traceback.print_exc()
#         # 返回一个包含错误信息的 Flask Response 对象
...
def summarize():
    try:
        ...
        # 将生成器包装在 Flask Response 中，并设置正确的 mimetype
        # 这会告诉浏览器，这是一个纯文本流
