...
# Import as custom_logger to avoid shadowing the logging module
...
if Compress is not None:
    app.config['COMPRESS_MIMETYPES'] = ['application/json', 'text/json', 'text/plain']
    app.config['COMPRESS_LEVEL'] = 6
    app.config['COMPRESS_MIN_SIZE'] = 500
    Compress(app)
else:
    logger.warning("flask_compress is not installed; responses will not be compressed")
...
@cross_origin()
def get_umap_points():
    # Support both `embedding` and `embeddingType` for frontend compatibility.
    embedding_type = (
        request.args.get("embedding")
        or request.args.get("embeddingType")
        or EMBED.SPECTER
    )
    ...
    umap_field = {
        ...
        "ada": "ada_umap",
        "glove": "glove_umap",
        "specter": "specter_umap",
    }.get(str(embedding_type).lower(), "specter_umap")

    # Add `x`/`y` flat coordinates for clients that don't read `*_umap` fields.
    normalized_points = []
    for p in points:
        ...
        if isinstance(coords, (list, tuple)) and len(coords) == 2:
            normalized_points.append({
                **p,
                ...
                "x": coords[0],
                "y": coords[1],
                "embedding": str(embedding_type).lower(),
            })
        else:
            normalized_points.append({
                **p,
    ...
    return jsonify(normalized_points)
